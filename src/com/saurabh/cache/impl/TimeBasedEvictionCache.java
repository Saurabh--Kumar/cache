...
public abstract class TimeBasedEvictionCache<K,V> implements Cache<K,V> {
    protected final Integer maxSize;
    protected Integer currentSize;
    protected ListNode<K,V> start;
    protected ListNode<K,V> end;
    protected final HashMap<K, ListNode<K,V>> valueNodePointerMap;

    public TimeBasedEvictionCache(Integer size) throws CacheInitialisationException {
        if(size <= 0){
